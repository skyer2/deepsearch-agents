...
规则引擎产出 IntentSlots 与 intent_confidence，供 LLM Planner 修正与 HITL 澄清。
"""
...
def infer_output_preference(
    ...
    *,
    ...
    if deliverable == "md":
        return "file_md"
    if any(k in q for k in ("不要文件", "不需要文件", "只要回答", "简要回答", "对话")):
        return "chat"
    # 【Phase 14】列出 N 条 + 来源 → 倾向结构化文件交付
    if item_count and require_citations:
        return "file_md"
    if require_citations and any(k in q for k in ("列出", "列举", "整理")):
        return "file_md"
    return "auto"
...
def compute_rule_confidence(
    *,
    ...
    if deliverable in {"md", "pdf"}:
        score += 0.15
    elif deliverable == "text" and slots.output_preference == "chat":
        score += 0.1
...
def detect_ambiguity_flags(
    ...
    *,
    ...
    has_file_hint = any(
        k in q for k in ("markdown", "Markdown", "MD", "md", "PDF", "pdf", "报告", "文件")
    )
    has_chat_hint = any(k in q for k in ("列出", "列举", "简要", "对话", "回答"))
    ...
    source_count = sum([needs_network, needs_database, needs_knowledge_base])
    if source_count >= 2:
...
def build_clarification_question(intent_deliverable: str, flags: list[str], slots: IntentSlots) -> str:
    ...
    if "deliverable_ambiguous" in flags or "output_preference_unclear" in flags:
        return (
            ...
            "（对话回答选 text / Markdown 文件选 md）"
        )
    if "multi_source" in flags:
        return (
            "本任务涉及多个信息源（网络/数据库/知识库）。"
            "请确认是否按系统建议的执行计划继续，或在审批面板中编辑步骤顺序。"
        )
    if intent_deliverable == "text" and slots.require_citations:
        return "请确认：是否需要将带引用的研究结果保存为 Markdown 文件？"
